...
class SHT30:
    ...
    * https://www.sensirion.com/fileadmin/user_upload/customers/sensirion/Dokumente/11_Sample_Codes_Software/Humidity_Sensors/Sensirion_Humidity_Sensors_SHT3x_Sample_Code_V2.pdf
    ...
    def __init__(self, i2c_addr=DEFAULT_I2C_ADDRESS, i2c_device=None, delta_temp=0, delta_hum=0):
        self.i2c = i2c_device
    ...
    # Reemplaza la función send_cmd original por esta
    def send_cmd(self, cmd_request, response_size=6, read_delay_ms=20):
        """
        Send a command to the sensor and read (optionally) the response.
        This version is adapted for modern MicroPython I2C API.
        """
        try:
            # La API moderna no necesita start() ni stop() para operaciones simples.
            # Primero, enviamos el comando para iniciar la medición.
            self.i2c.writeto(self.i2c_addr, cmd_request)

            # Si el comando no espera respuesta (como un reset), terminamos aquí.
            if not response_size:
                return

            # Esperamos el tiempo necesario para que el sensor tome la medida.
            time.sleep_ms(read_delay_ms)

            # Leemos el número de bytes de la respuesta.
            data = self.i2c.readfrom(self.i2c_addr, response_size)

            # Comprobamos el CRC de los datos recibidos.
            # La respuesta viene en bloques de 3 bytes: [Dato_MSB, Dato_LSB, CRC]
            for i in range(response_size // 3):
                if not self._check_crc(data[i*3:(i+1)*3]):
                    raise SHT30Error(SHT30Error.CRC_ERROR)

            # Comprobamos si la respuesta son solo ceros (un error común)
            ...
            # Si hay un error de hardware en el bus, lo capturamos.
